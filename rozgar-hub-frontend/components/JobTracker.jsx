...
export default function JobTracker({ role, job, worker, employer, agreedPrice, socket, onJobComplete, lang = "en", t = {} }) {
  ...
  useEffect(() => {
    console.log("🎬 JobTracker Mounted | Role:", role, "Socket:", socket?.connected);
  }, []);
  ...
  const [workerOwnLocation, setWorkerOwnLocation]   = useState(null);
  ...
  // ✅ FIX 1: showOtpModal drives hiding the map so route badge can't bleed through
  ...
  useEffect(() => {
    ...
    socket.on("payment_otp_sent",        () => {
      if (role === "worker") setShowOtpModal(true);
    });
      ...
      socket.on("worker_location_update", async (data) => {
        if (data.workerId === worker?.workerId) {
          ...
          try {
            ...
            setWorkerAddress(road ? `${road}${area ? ", " + area : ""}` : json.display_name?.split(",")[0] || "");
          } catch (_) {}
        }
      });
    }
  ...
  // Worker GPS broadcast
  useEffect(() => {
    if (role !== "worker" || !socket || currentStep === "completed") return;
    if (!navigator.geolocation || !worker?.workerId || !employer?.employerId) return;

    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        const { latitude, longitude } = pos.coords;
        setWorkerOwnLocation({ lat: latitude, lng: longitude });
      ...
      (err) => { if (err.code === 1) console.warn("⚠️ Location permission denied."); },
  ...
  // ✅ FIX 2: Auto-trigger onJobComplete when step becomes "completed"
  useEffect(() => {
    if (currentStep !== "completed") return;
    const timer = setTimeout(() => onJobComplete && onJobComplete(), 1800);
  ...
  // ✅ FIX 1: Hide map when OTP modal is open so route badge can't show through
  const shouldShowMap = !isCompleted && job?.lat && job?.lng && !showOtpModal;
  ...
  return (
    <div style={{ maxWidth:580, margin:"0 auto", fontFamily:lang==="ur"?"'Noto Nastaliq Urdu',serif":"'Outfit',sans-serif", position:"relative" }}>
      ...
      {/* HEADER */}
      <div style={{ background:"linear-gradient(135deg,#0b1526 0%,#1a3255 100%)", borderRadius:22, padding:"24px 24px 20px", marginBottom:14, boxShadow:"0 8px 32px rgba(11,21,38,.28)" }}>
        ...
        <div style={{ display:"flex", gap:10, marginTop:18 }}>
          ...
          {role === "employer" && distanceEta && currentStep !== "completed" && !showOtpModal && (
      ...
      {/* OTHER PARTY CARD */}
      <div style={{ background:"#fff", borderRadius:18, padding:"16px 20px", marginBottom:14, boxShadow:"0 2px 14px rgba(0,0,0,.06)" }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            {otherPhone && (
              <a href={`tel:${otherPhone}`}
                ...
                <Phone size={14} />{lang==="ur"?"کال":"Call"}
              </a>
            )}
            <button onClick={() => { setShowChat(true); setUnreadCount(0); }}
              ...
              <MessageCircle size={14} />{lang==="ur"?"چیٹ":"Chat"}
            </button>
          </div>
        </div>

        {role === "employer" && workerAddress && currentStep !== "completed" && !showOtpModal && (
      ...
      {/* ✅ FIX 1: Live Map — hidden when OTP modal is open */}
      {shouldShowMap && (
        <div style={{ background:"#fff", borderRadius:18, padding:"20px", marginBottom:14, boxShadow:"0 2px 14px rgba(0,0,0,.06)" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
          ...
          <JobMap
            ...
            workerOwnLocation={role === "worker" ? workerOwnLocation : null}
          />
          {role === "worker" && (
      ...
      {/* ✅ FIX 1: OTP Modal — z-index 9000, completely isolated from map layer */}
      {role === "worker" && showOtpModal && (
        <div style={{
          position:"fixed", inset:0, background:"rgba(0,0,0,.75)",
          display:"flex", alignItems:"center", justifyContent:"center",
          // ✅ Higher z-index than map route badge (which is z-index 1000)
          zIndex:9000,
          padding:24, backdropFilter:"blur(8px)",
        }}>
          <div style={{ background:"#fff", borderRadius:24, padding:"32px 28px", maxWidth:380, width:"100%", textAlign:"center", animation:"pop .35s ease-out", position:"relative", zIndex:9001 }}>
      ...
      {isCompleted && (
        <div style={{ background:"#fff", borderRadius:18, padding:"28px 24px", boxShadow:"0 2px 14px rgba(0,0,0,.06)", textAlign:"center", animation:"pop .45s ease-out" }}>
          ...
          <h3 style={{ fontSize:19, fontWeight:900, color:"#0f172a", marginBottom:6 }}>{lang==="ur"?"نوکری مکمل!":"Job Completed!"}</h3>
          <p style={{ color:"#64748b", fontSize:13.5 }}>{lang==="ur"?"شکریہ، آپ کی رائے لی جا رہی ہے۔":"Thank you! Loading your review…"}</p>
        </div>
      )}
          ...
          <div style={{ background:"#fff", borderRadius:24, width:"100%", maxWidth:420, height:"80vh", display:"flex", flexDirection:"column", boxShadow:"0 24px 60px rgba(0,0,0,.3)", animation:"slideUp2 .3s ease-out" }}>
            <div style={{ background:"linear-gradient(135deg,#0f172a,#1e3a5f)", padding:"18px 20px", borderRadius:"24px 24px 0 0", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize:11, color:"rgba(255,255,255,.4)" }}>{chatLang==="ur"?"آن لائن":"Online"}</div>
                </div>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                <button onClick={() => setChatLang(chatLang === "en" ? "ur" : "en")}
                  style={{ background:"rgba(255,255,255,.15)", border:"1px solid rgba(255,255,255,.3)", borderRadius:8, padding:"6px 12px", cursor:"pointer", color:"#fff", fontSize:11, fontWeight:700 }}>
                  {chatLang === "en" ? "اردو" : "English"}
                </button>
            ...
            <div style={{ flex:1, overflowY:"auto", padding:"16px 16px 8px", display:"flex", flexDirection:"column", gap:10 }}>
              ...
              {messages.map((msg, i) => {
                ...
                return (
                  <div key={i} className="chat-msg-in" style={{ display:"flex", flexDirection:"column", alignItems:isMine?"flex-end":"flex-start" }}>
