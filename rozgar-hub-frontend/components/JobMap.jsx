...
// ── Internal map renderer (reused for small + fullscreen) ─────
function LeafletMapRenderer({ lat, lng, height, onLocationSelect, showWorkerLocation, workerId, employerId, showLiveWorkerLocation, liveWorkerId, workerOwnLocation }) {
  const mapRef = useRef(null);
  const mapInstanceRef = useRef(null);
  const jobMarkerRef = useRef(null);
  const workerMarkerRef = useRef(null);
  ...
  // Calculate straight-line distance (haversine) for ETA fallback
  const calculateDistance = (lat1, lng1, lat2, lng2) => {
    const R = 6371;
    const dLat = (lat2 - lat1) * Math.PI / 180;
    const dLng = (lng2 - lng1) * Math.PI / 180;
  ...
  // Fetch actual road route from OSRM and draw a polyline on the map.
  // Throttled to once per 8 seconds to avoid hammering the public API.
  // For production, replace with a self-hosted OSRM instance or paid routing API.
  const fetchAndDrawRoute = useCallback(async (fromLat, fromLng, toLat, toLng) => {
    const now = Date.now();
    ...
    try {
      ...
      if (!res.ok || !mapInstanceRef.current) return;
      const data = await res.json();
      if (!data.routes?.[0] || !mapInstanceRef.current) return;
      const L = window.L;
      // OSRM returns [lng, lat]; Leaflet needs [lat, lng]
      const coords = data.routes[0].geometry.coordinates.map(([cLng, cLat]) => [cLat, cLng]);
  ...
  useEffect(() => {
    ...
    const initMap = () => {
      ...
      // Employer view: initial worker marker if location already known
      if (showLiveWorkerLocation && workerLocation) {
        const liveWorkerIcon = L.divIcon({
          html: `<div style="position:relative;width:32px;height:32px"><div style="position:absolute;inset:0;background:rgba(34,197,94,0.25);border-radius:50%;animation:gps-pulse 2s ease-out infinite"></div><div style="position:absolute;inset:0;background:rgba(34,197,94,0.15);border-radius:50%;animation:gps-pulse 2s ease-out 0.6s infinite"></div><div style="position:absolute;inset:6px;background:#22c55e;border-radius:50%;border:3px solid #fff;box-shadow:0 2px 10px rgba(34,197,94,0.8)"></div></div>`,
          className: "", iconSize: [32, 32], iconAnchor: [16, 16],
        });
        liveWorkerMarkerRef.current = L.marker([workerLocation.lat, workerLocation.lng], { icon: liveWorkerIcon }).addTo(map).bindPopup(`<b>🟢 Worker Location</b>`);
        if (lat && lng) {
          map.fitBounds([[workerLocation.lat, workerLocation.lng], [lat, lng]], { padding: [50, 50] });
          fetchAndDrawRoute(workerLocation.lat, workerLocation.lng, lat, lng);
        }
      }

      // Worker view: show worker's own location and route to job
      if (showWorkerLocation && workerOwnLocation) {
        const workerSelfIcon = L.divIcon({
          html: `<div style="position:relative;width:28px;height:28px"><div style="position:absolute;inset:0;background:rgba(34,197,94,0.25);border-radius:50%;animation:gps-pulse 2s ease-out infinite"></div><div style="position:absolute;inset:0;background:rgba(34,197,94,0.15);border-radius:50%;animation:gps-pulse 2s ease-out 0.6s infinite"></div><div style="position:absolute;inset:6px;background:#22c55e;border-radius:50%;border:2.5px solid #fff;box-shadow:0 2px 10px rgba(34,197,94,0.8)"></div></div>`,
          className: "", iconSize: [28, 28], iconAnchor: [14, 14],
        });
        workerMarkerRef.current = L.marker([workerOwnLocation.lat, workerOwnLocation.lng], { icon: workerSelfIcon })
          .addTo(map).bindPopup("<b>📍 Your Location</b>");
        if (lat && lng) {
          map.fitBounds([[workerOwnLocation.lat, workerOwnLocation.lng], [lat, lng]], { padding: [60, 60] });
          fetchAndDrawRoute(workerOwnLocation.lat, workerOwnLocation.lng, lat, lng);
        }
      }

      // Employer: attach socket listener for live worker location updates
      if (showLiveWorkerLocation && window._rozgarSocket) {
        const handleLocationUpdate = async (data) => {
          console.log("🗺️ JobMap received worker_location_update:", data);
          if (data.workerId === liveWorkerId) {
  ...
  // Employer: update worker marker + draw road route when live location changes
  useEffect(() => {
    if (!mapInstanceRef.current || !showLiveWorkerLocation || !workerLocation) return;
    const L = window.L;
    if (liveWorkerMarkerRef.current) {
    ...
    if (lat && lng) fetchAndDrawRoute(workerLocation.lat, workerLocation.lng, lat, lng);
  }, [workerLocation, eta, lat, lng, showLiveWorkerLocation, workerAddress, fetchAndDrawRoute]);

  // Worker: update own location marker + draw road route to job when GPS changes
  useEffect(() => {
    if (!mapInstanceRef.current || !showWorkerLocation || !workerOwnLocation || !window.L) return;
    const L = window.L;
    if (workerMarkerRef.current) {
      workerMarkerRef.current.setLatLng([workerOwnLocation.lat, workerOwnLocation.lng]);
    } else {
      const workerSelfIcon = L.divIcon({
        html: `<div style="position:relative;width:28px;height:28px"><div style="position:absolute;inset:0;background:rgba(34,197,94,0.25);border-radius:50%;animation:gps-pulse 2s ease-out infinite"></div><div style="position:absolute;inset:0;background:rgba(34,197,94,0.15);border-radius:50%;animation:gps-pulse 2s ease-out 0.6s infinite"></div><div style="position:absolute;inset:6px;background:#22c55e;border-radius:50%;border:2.5px solid #fff;box-shadow:0 2px 10px rgba(34,197,94,0.8)"></div></div>`,
        className: "", iconSize: [28, 28], iconAnchor: [14, 14],
    ...
    if (lat && lng) {
      mapInstanceRef.current.fitBounds(
        [[workerOwnLocation.lat, workerOwnLocation.lng], [lat, lng]],
        { padding: [60, 60] }
      );
      fetchAndDrawRoute(workerOwnLocation.lat, workerOwnLocation.lng, lat, lng);
    }
  }, [workerOwnLocation, lat, lng, showWorkerLocation, fetchAndDrawRoute]);
    ...
    <div style={{ height: height || "100%", width: "100%", position: "relative" }}>
      ...
      {routeInfo && (
        <div style={{
          ...
          padding: "7px 14px", boxShadow: "0 2px 10px rgba(0,0,0,.18)",
          border: "1.5px solid #bfdbfe", display: "flex", gap: 12,
          alignItems: "center", fontSize: 12, fontWeight: 700,
          pointerEvents: "none",
        }}>
          <span style={{ color: "#3b82f6" }}>🗺️ {routeInfo.distance} km</span>
          <span style={{ color: "#64748b" }}>⏱ ~{routeInfo.duration} min</span>
        </div>
      )}
    </div>
  );
    ...
    <>
      ...
      <div style={{ borderRadius: 12, overflow: "hidden", border: "1.5px solid #e2e8f0" }}>
        ...
        <LeafletMapRenderer
          ...
          showLiveWorkerLocation={showLiveWorkerLocation}
          liveWorkerId={liveWorkerId}
          workerOwnLocation={workerOwnLocation}
        />
      </div>
