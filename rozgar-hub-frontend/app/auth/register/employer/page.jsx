      ...
      <div className="max-w-lg w-full bg-white/95 backdrop-blur-md p-8 rounded-2xl shadow-2xl space-y-6 border border-white/20">
        ...
        <div className="flex items-center gap-2 justify-center mb-6">
          {STEPS.map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${step === s ? "bg-green-600 text-white" : STEPS.indexOf(step) > i ? "bg-green-500 text-white" : "bg-gray-200 text-gray-400"}`}>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => setLang("en")} className={`px-3 py-1 rounded-lg text-sm font-semibold transition-all ${lang === "en" ? "bg-green-600 text-white" : "bg-gray-200 text-gray-600 hover:bg-gray-300"}`}>EN</button>
            <button onClick={() => setLang("ur")} className={`px-3 py-1 rounded-lg text-sm font-semibold transition-all ${lang === "ur" ? "bg-green-600 text-white" : "bg-gray-200 text-gray-600 hover:bg-gray-300"}`}>اردو</button>
          </div>
        </div>
        ...
        {step === "form" && (
          <form onSubmit={handleFormNext} className="space-y-4">
            ...
            <button type="submit" className="w-full py-3 bg-green-600 text-white rounded-xl font-semibold hover:bg-green-700 transition shadow-md">{t.continue}</button>
            <p className="text-center text-sm text-gray-500">
              {lang === "ur" ? "پہلے سے اکاؤنٹ ہے؟" : "Already have an account?"}{" "}
              <button type="button" onClick={() => router.push("/auth/login")} className="text-green-600 font-semibold hover:underline">
        ...
        {step === "docs" && (
          <form onSubmit={handleDocsSubmit} className="space-y-5">
            ...
            <div className="flex gap-3">
              ...
              <button type="submit" disabled={loading} className="flex-1 py-3 bg-green-600 text-white rounded-xl font-semibold hover:bg-green-700 transition shadow-md disabled:opacity-60 disabled:cursor-not-allowed">
          ...
          <form onSubmit={handleVerifyOTP} className="space-y-5">
            ...
            <div>
              ...
              <input type="text" inputMode="numeric" maxLength={6} placeholder="• • • • • •" value={otp}
                ...
                className="w-full border p-3 rounded-lg outline-none focus:ring-2 focus:ring-green-500 transition text-center text-2xl font-bold tracking-widest border-gray-300 focus:border-green-500"
              />
              <p className="text-xs text-gray-400 mt-1 text-center">{t.otpExpires}</p>
            </div>
            <button type="submit" disabled={otpLoading} className="w-full py-3 bg-green-600 text-white rounded-xl font-semibold hover:bg-green-700 transition shadow-md disabled:opacity-60 disabled:cursor-not-allowed">
              {otpLoading ? t.verifying : t.verify}
            </button>
            <div className="flex justify-between text-sm">
              <button type="button" onClick={handleResend} className="text-green-600 hover:underline">{t.resendOTP}</button>
...
function Input({ error, ...props }) {
  return (
    <div>
      <input {...props} className={`w-full border p-3 rounded-lg outline-none transition focus:ring-2 focus:ring-green-500 ${error ? "border-red-500" : "border-gray-300 focus:border-green-500"}`} />
...
function FileUpload({ label, hint, accept, preview, error, onChange }) {
  return (
    <div>
      ...
      <label className={`flex flex-col items-center justify-center w-full border-2 border-dashed rounded-xl cursor-pointer transition ${error ? "border-red-400 bg-red-50" : "border-gray-300 bg-gray-50 hover:border-green-500 hover:bg-green-50"}`} style={{ minHeight: preview ? "auto" : "100px" }}>
        {preview ? (
          <div className="w-full p-2">
            <img src={preview} alt={label} className="w-full max-h-40 object-contain rounded-lg" />
            <p className="text-xs text-center text-green-600 mt-2 font-medium">✓ Uploaded — click to change</p>
          </div>
        ) : (
