...
const translations = {
  ...
  ur: {
    ...
    newPassPlaceholder: "کم از کم 6 حروف",
    ...
    passMinLength: "پاس ورڈ کم از کم 6 حروف کا ہونا چاہیے۔",
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input
            ...
            className="w-full bg-white border border-gray-300 p-3 rounded-lg outline-none focus:ring-2 focus:ring-orange-400 focus:border-orange-400 transition"
            required
          />
        </div>
    ...
    <div className="space-y-5">
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm mt-1">
          {t.otpSentTo} <span className="font-semibold text-orange-500">{maskedEmail}</span>
        </p>
      </div>
        ...
        {otp.map((digit, index) => (
          <input
            ...
            className={`w-11 h-12 text-center text-xl font-bold border-2 rounded-xl outline-none transition-all bg-white
              ${digit ? "border-orange-500 bg-orange-50 text-orange-600" : "border-gray-200"}
              focus:border-orange-500 focus:bg-white ${error ? "border-red-300" : ""}`}
          />
        ))}
      </div>
      ...
      <p className="text-center text-gray-500 text-sm">
        {t.didntReceive}{" "}
        {canResend ? (
          <button onClick={handleResend} disabled={resending} className="text-orange-500 font-semibold hover:underline disabled:opacity-50">
            {resending ? t.resending : t.resendOtp}
          </button>
        ) : (
          <span className="text-gray-400">
            {t.resendIn} <span className="font-semibold text-orange-500">{countdown}s</span>
          </span>
        )}
      </p>
    </div>
...
function StepNewPassword({ email, otp, onDone, t }) {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    if (newPassword.length < 6) { setError(t.passMinLength); return; }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-white border border-gray-300 p-3 pr-10 rounded-lg outline-none focus:ring-2 focus:ring-orange-400 focus:border-orange-400 transition"
            required
          />
      ...
      <div className="absolute top-20 right-6 flex gap-2 z-20">
        <button
          ...
          className={`px-3 py-1 rounded-lg font-semibold transition-all ${
            lang === "en" ? "bg-orange-500 text-white" : "bg-white/10 text-gray-300 hover:bg-white/20"
          }`}
        >
          EN
        </button>
        <button
          onClick={() => setLang("ur")}
          className={`px-3 py-1 rounded-lg font-semibold transition-all ${
            lang === "ur" ? "bg-orange-500 text-white" : "bg-white/10 text-gray-300 hover:bg-white/20"
          }`}
        >
          اردو
        </button>
      ...
      <div className="w-full max-w-md rounded-3xl shadow-2xl p-10" style={{ backgroundColor: "#e8edf5" }}>
        ...
        <div className="flex justify-center gap-2 mb-8">
          {[1, 2, 3, 4].map((s) => (
            <div key={s} className={`h-2 rounded-full transition-all duration-300 ${
              s === step ? "w-8 bg-orange-500" : s < step ? "w-2 bg-orange-300" : "w-2 bg-gray-300"
            }`} />
          ))}
        </div>
        ...
        {step < 4 && (
          <p className="text-center text-gray-500 text-sm mt-6">
            {t.rememberPass}{" "}
            <span onClick={() => router.push("/auth/login")} className="text-orange-500 cursor-pointer font-medium hover:underline">
              {t.backToLogin}
            </span>
          </p>
        )}
