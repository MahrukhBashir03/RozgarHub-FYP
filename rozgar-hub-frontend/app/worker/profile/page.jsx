...
// //     // ✅ Mark availabilityPosted on backend user profile
// //     try {
...
// //         },
// //         body: JSON.stringify({ availabilityPosted: true }),
// //       });
// //     } catch (_) {}

// //     // ✅ Re-fetch full profile so points update
// //     if (token) {
...
// //     if (res.ok) {
// //       swal({ title: t.swalPosted, text: t.swalPostedText, icon: "success" });
// //     } else {
// //       swal({ title: t.swalError, text: t.swalErrorText, icon: "error" });
// //     }
// //   } catch (_) {
// //     setShowAvailModal(false);
// //     setUserProfile((prev) => prev ? { ...prev, availabilityPosted: true } : prev);
// //     swal({ title: t.swalSavedLocally, text: t.swalSavedLocallyText, icon: "info" });
// //   }
// // };
...
// //   // ✅ Support both nested documents and flat fields
...
// //   );
// //   const isAvailPosted = !!(
// //     userProfile.availabilityPosted ||
// //     userProfile.availabilityId
// //   );
...
// //     ...(isDriver
// //       ? [{ key: "license", label: t.stepLicense, done: hasLicense, points: 10, action: null }]
// //       : []),
...
// //           <div key={step.key} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", borderRadius: 12, background: step.done ? "#f0fdf4" : "#f8fafc", border: `1.5px solid ${step.done ? "#bbf7d0" : "#e2e8f0"}` }}>
// //             <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
// //               <div style={{ width: 28, height: 28, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: step.done ? "#22c55e" : "#e2e8f0", fontSize: 13, color: step.done ? "#fff" : "#94a3b8" }}>
// //                 {step.done ? "✓" : "○"}
// //               </div>
// //               <span style={{ fontSize: 13, fontWeight: step.done ? 600 : 500, color: step.done ? "#15803d" : "#475569" }}>{step.label}</span>
// //             </div>
// //             <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
// //               <span style={{ fontSize: 11, fontWeight: 700, color: step.done ? "#16a34a" : "#94a3b8", background: step.done ? "#dcfce7" : "#f1f5f9", padding: "2px 8px", borderRadius: 20 }}>
// //                 +{step.points} pts
// //               </span>
// //               {!step.done && step.action && (
// //                 <button onClick={step.action} style={{ fontSize: 11, fontWeight: 700, color: "#3b82f6", background: "#eff6ff", border: "none", borderRadius: 20, padding: "3px 10px", cursor: "pointer" }}>
// //                   {t.tapToComplete} →
// //                 </button>
// //               )}
// //             </div>
...
// // // Add this useEffect alongside the others in WorkerDashboard / EmployerDashboard
// // useEffect(() => {
...
//   en: {
//     dir: "ltr", brand: "RozgarHub", role: "WORKER",
//     navRequests: "Job Requests", navApplied: "My Applications", navBrowse: "Browse Jobs",
//     navHistory: "Job History", navAvail: "Post Availability",
//     logout: "Logout", online: "Online",
//     headerRequests: "Incoming Requests", headerApplied: "My Applications",
...
//     stepCnicDocs: "CNIC Documents Uploaded", stepAvailability: "Availability Posted",
//     stepLicense: "Driving License Uploaded", stepVerified: "Admin Verified",
//     tapToComplete: "Tap to complete", trackingHeader: "Job In Progress",
//   },
//   ur: {
//     dir: "rtl", brand: "روزگار ہب", role: "ورکر",
//     navRequests: "نوکری کی درخواستیں", navApplied: "میری درخواستیں", navBrowse: "نوکریاں دیکھیں",
//     navHistory: "نوکری کی تاریخ", navAvail: "دستیابی پوسٹ کریں",
//     logout: "لاگ آؤٹ", online: "آن لائن",
//     headerRequests: "آنے والی درخواستیں", headerApplied: "میری درخواستیں",
...
//     stepCnicDocs: "شناختی کارڈ اپلوڈ", stepAvailability: "دستیابی پوسٹ",
//     stepLicense: "ڈرائیونگ لائسنس اپلوڈ", stepVerified: "ایڈمن سے تصدیق",
//     tapToComplete: "مکمل کریں", trackingHeader: "نوکری جاری ہے",
//   }
// };
...
//     if (!tk) return;
//     fetch("http://localhost:5000/api/auth/me", { headers: { Authorization: `Bearer ${tk}` } })
//       .then(r => r.json())
//       .then(data => { if (data.user) setUserProfile(data.user); })
//       .catch(() => {});
//   };
...
//     /* Job-request history (accepted/completed via popup) */
//     const headers = token ? { Authorization: `Bearer ${token}` } : {};
//     fetch(`http://localhost:5000/api/job-requests/worker/${u.id}`, { headers })
//       .then(r => r.json())
//       .then(data => {
//         if (!Array.isArray(data)) return;
//         /* Accepted / confirmed → My Applications */
//         const accepted = data.filter(r => ["accepted","confirmed","in_progress","completed"].includes(r.status));
//         setAppliedJobs(prev => {
//           const ids = new Set(prev.map(x => x._id));
//           const merged = [...prev];
...
//           if (exists) return prev.map(a => a._id === incomingRequest.requestId ? { ...a, status:"confirmed" } : a);
...
//     socket.on("request_taken_not_selected", () => {
//       if (popupState === "accepted_waiting" || popupState === "counter_sent") setPopupState("rejected");
//       else if (!showPopup) { setShowPopup(false); setIncomingRequest(null); }
//     });
...
//     socket.on("employer_counter", (data) => {
//       setEmployerCounter(data); setPopupState("counter_received");
//     });
...
//     return () => {
//       socket.off("new_job_request"); socket.off("employer_confirm_worker");
//       socket.off("employer_accepted"); socket.off("request_taken_not_selected");
//       socket.off("employer_rejected"); socket.off("employer_counter"); socket.off("employer_dismiss_worker");
//     };
//   }, [user, incomingRequest, popupState, showPopup, showTracker, counterPrice]);
...
//       setPopupTimer(30);
//       timerRef.current = setInterval(() => {
//         setPopupTimer(prev => { if (prev <= 1) { clearInterval(timerRef.current); dismissPopup(); return 0; } return prev - 1; });
//       }, 1000);
//     } else clearInterval(timerRef.current);
//     return () => clearInterval(timerRef.current);
//   }, [showPopup, popupState]);
...
//   const dismissPopup = () => {
//     setShowPopup(false); setPopupState(null); setCounterPrice(""); setEmployerCounter(null);
//   };
...
//   const handleAcceptJob = () => {
//     socket.emit("worker_job_accept", { requestId: incomingRequest.requestId, employerId: incomingRequest.employerId, workerId: user.id, workerName: user.name, workerRating: "4.8", workerPhone: user.phone || "" });
//     setPopupState("accepted_waiting");
//   };
...
//   const handleSendCounter = () => {
//     if (!counterPrice) { swal({ title: "Enter a price", text: "Please enter a counter price.", icon: "warning" }); return; }
//     socket.emit("worker_offer", { requestId: incomingRequest.requestId, employerId: incomingRequest.employerId, workerId: user.id, workerName: user.name, price: counterPrice, rating: "4.8", phone: user.phone });
//     setPopupState("counter_sent");
//   };

//   const handleAcceptEmployerCounter = () => {
//     socket.emit("worker_accepted", { requestId: incomingRequest.requestId, employerId: incomingRequest.employerId, workerId: user.id, workerName: user.name, price: employerCounter.price, phone: user.phone });
//     setConfirmedJob({ request: incomingRequest, price: employerCounter.price });
//     setPopupState("confirmed");
//   };
...
//   const handleRejectEmployerCounter = () => {
//     swal({ title: t.swalRejectCounter, text: t.swalRejectCounterText, icon: "warning", showCancelButton: true, confirmButtonColor: "#ef4444", cancelButtonColor: "#6b7280", confirmButtonText: t.swalYesReject, cancelButtonText: t.swalGoBack })
//       .then(r => { if (r.isConfirmed) { socket.emit("worker_rejected_counter", { requestId: incomingRequest.requestId, employerId: incomingRequest.employerId, workerId: user.id }); dismissPopup(); setIncomingRequest(null); } });
//   };

//   /* ── Post Availability — saves to backend + refetches profile ── */
//   const handlePostAvailability = async (e) => {
//     e.preventDefault();
//     const token = localStorage.getItem("token");
//     try {
...
//       /* ── Persist availabilityPosted flag to user profile ── */
//       try {
//         await fetch("http://localhost:5000/api/auth/update-profile", {
//           method: "PATCH",
...
//       /* ── Re-fetch so points update immediately ── */
//       if (token) {
//         refetchProfile(token);
//       } else {
//         setUserProfile(prev => prev ? { ...prev, availabilityPosted: true } : prev);
...
//     activeTab === "requests" ? t.headerRequests :
//     activeTab === "applied"  ? t.headerApplied  :
//     activeTab === "history"  ? t.headerHistory  :
//     t.headerBrowse;
...
//       {showPopup && incomingRequest && (
//         <JobRequestPopup
//           t={t} request={incomingRequest} popupState={popupState} popupTimer={popupTimer}
...
//                 <div style={{ background:"#f8fafc", border:"1.5px solid #e2e8f0", borderRadius:12, padding:"12px 16px", marginBottom:20, display:"flex", alignItems:"center", gap:8 }}>
//                   <MapPin size={15} color="#3b82f6" /><span style={{ fontSize:14, color:"#475569", fontWeight:600 }}>{selectedJob.location}</span>
//                 </div>
//               )}
//               <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:10, marginBottom:24 }}>
//                 <label style={{ fontSize:13, fontWeight:700, color:"#374151" }}>{t.yourOffer}</label>
//                 <div style={{ display:"flex", alignItems:"center", gap:12 }}>
//                   <button onClick={() => setOfferedRate(prev=>String(Math.max(0,Number(prev)-100)))} style={{ width:42, height:42, borderRadius:10, border:"none", background:"#f1f5f9", fontSize:22, fontWeight:700, cursor:"pointer" }}>−</button>
//                   <input type="number" value={offeredRate} onChange={e=>setOfferedRate(e.target.value)} style={{ width:130, textAlign:"center", border:"2px solid #16a34a", borderRadius:12, padding:"10px 0", fontSize:22, fontWeight:800, color:"#0f172a", outline:"none" }} />
//                   <button onClick={() => setOfferedRate(prev=>String(Number(prev)+100))} style={{ width:42, height:42, borderRadius:10, border:"none", background:"#f1f5f9", fontSize:22, fontWeight:700, cursor:"pointer" }}>+</button>
//                 </div>
//                 <p style={{ fontSize:12, color:"#94a3b8", margin:0 }}>
//                   {Number(offeredRate)>Number(selectedJob.salary)?`↑ Rs. ${Number(offeredRate)-Number(selectedJob.salary)} ${t.aboveRate}`:Number(offeredRate)<Number(selectedJob.salary)?`↓ Rs. ${Number(selectedJob.salary)-Number(offeredRate)} ${t.belowRate}`:t.matchingRate}
//                 </p>
...
//           {!showTracker && activeTab === "browse"   && (
//             <BrowseTab t={t} lang={lang} userId={user.id} onOpenApplyModal={(job) => { setSelectedJob(job); setOfferedRate(String(job.salary||"")); setShowApplyModal(true); }} />
//           )}
//         </div>
//       </main>
...
//         <p style={{ fontSize:14 }}>Jobs you apply to and requests you accept will appear here.</p>
//       </div>
//     );
//   }
...
//   const statusStyle = (status) => {
//     if (status === "completed")  return { bg:"#dcfce7", color:"#15803d" };
//     if (["confirmed","accepted","in_progress"].includes(status)) return { bg:"#dbeafe", color:"#1d4ed8" };
//     if (status === "shortlisted") return { bg:"#fef3c7", color:"#d97706" };
//     if (status === "rejected")   return { bg:"#fee2e2", color:"#dc2626" };
//     return { bg:"#f1f5f9", color:"#475569" };
//   };
...
//               </div>
//               <span style={{ padding:"6px 14px", borderRadius:20, fontSize:12, fontWeight:700, background:bg, color, flexShrink:0, marginLeft:12 }}>{(app.status||"pending").toUpperCase().replace("_"," ")}</span>
//             </div>
//             {(app.offeredRate||app.agreedPrice) && (
//               <div style={{ marginTop:12, display:"inline-flex", alignItems:"center", gap:6, padding:"8px 14px", background:"#f0fdf4", borderRadius:10 }}>
//                 <span style={{ fontSize:13, color:"#16a34a", fontWeight:700 }}>💰 {app.agreedPrice?"Agreed:":"Your Offer:"} Rs. {app.agreedPrice||app.offeredRate}</span>
//               </div>
//             )}
//           </div>
//         );
...
//     );
//   }
//   const totalEarned = history.reduce((sum, j) => sum + Number(String(j.agreedPrice).replace(/[^0-9]/g,"") || 0), 0);
//   return (
//     <div style={{ maxWidth:700, margin:"0 auto" }}>
...
//         <div>
//           <div style={{ fontSize:12, opacity:0.6, fontWeight:600, textTransform:"uppercase", letterSpacing:".08em" }}>Jobs Completed</div>
//           <div style={{ fontSize:32, fontWeight:900 }}>{history.length}</div>
//         </div>
//         <div style={{ textAlign:"right" }}>
//           <div style={{ fontSize:12, opacity:0.6, fontWeight:600, textTransform:"uppercase", letterSpacing:".08em" }}>Total Earned</div>
//           <div style={{ fontSize:24, fontWeight:800 }}>Rs. {totalEarned.toLocaleString()}</div>
//         </div>
//       </div>
//       <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
//         {history.map((job, i) => (
...
//   const applyFilters = (filters) => {
//     let results = [...allJobs];
//     if (filters.category) results = results.filter(j => (j.category||j.type||"").toLowerCase().includes(filters.category.toLowerCase())||(j.title||"").toLowerCase().includes(filters.category.toLowerCase()));
//     if (filters.location) results = results.filter(j => (j.location||"").toLowerCase().includes(filters.location.toLowerCase()));
//     if (filters.minBudget>0) results = results.filter(j => Number(String(j.salary).replace(/[^0-9]/g,""))>=filters.minBudget);
//     if (filters.maxBudget>0) results = results.filter(j => Number(String(j.salary).replace(/[^0-9]/g,""))<=filters.maxBudget);
//     if (filters.keywords?.length>0) results = results.filter(j => filters.keywords.some(kw=>(j.title||"").toLowerCase().includes(kw.toLowerCase())||(j.description||"").toLowerCase().includes(kw.toLowerCase())));
//     setFilteredJobs(results);
//   };
...
//               <h3 style={{ fontSize:20, fontWeight:800, color:"#0f172a", marginBottom:6 }}>{t.jobConfirmed}</h3>
//               <p style={{ fontSize:13, color:"#64748b", marginBottom:20 }}>{t.jobConfirmedDesc}</p>
//               <button onClick={onOpenTracker} style={{ padding:"13px 32px", background:"linear-gradient(135deg,#22c55e,#16a34a)", color:"#fff", border:"none", borderRadius:12, fontSize:15, fontWeight:700, cursor:"pointer", width:"100%", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}>
//                 🔧 {t.viewJobDetails}
//               </button>
//             </div>
//           )}
...
export default function WorkerDashboard() {
  ...
  useEffect(() => {
    ...
    socket.on("employer_counter",  (data) => { setEmployerCounter(data); setPopupState("counter_received"); });
    socket.on("employer_dismiss_worker", () => { setPopupState("dismissed"); });
      ...
      <main style={{ flex:1, overflow:"auto" }}>
        <header style={{ background:"#fff", borderBottom:"1px solid #e2e8f0", padding:"20px 32px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:5 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            ...
            {notifications.length > 0 && <div style={{ position:"relative", cursor:"pointer" }} onClick={() => setNotifications([])}><Bell size={20} color="#64748b" /><div style={{ position:"absolute", top:-4, right:-4, width:16, height:16, background:"#ef4444", borderRadius:"50%", fontSize:10, color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700 }}>{notifications.length}</div></div>}
          </div>
        </header>
