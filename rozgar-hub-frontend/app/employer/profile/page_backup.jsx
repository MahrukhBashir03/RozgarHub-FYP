...
// // //                 style={{ background: "#f1f5f9", border: "none", borderRadius: 10, padding: 10, cursor: "pointer", transition: "all 0.2s" }}>
...
// // //                 style={{ padding: "8px 16px", borderRadius: 10, border: "2px solid #3b82f6", background: "#eff6ff", color: "#3b82f6", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: lang === "en" ? "'Noto Nastaliq Urdu', serif" : "'Outfit', sans-serif", transition: "all 0.2s" }}>
// // //                 {t.langBtn}
// // //               </button>
...
// // //                   <Bell size={18} color={notifications.length ? "#3b82f6" : "#94a3b8"} />
// // //                 </div>
...
// // //           <div className="main-pad" style={{ padding: "24px 26px", marginTop: 0, margin: 0 }}>
// // //             {activeTab === "find" && (
// // //               <FindWorkerFlow
// // //                 t={t} lang={lang}
...
// // // /* ═══════════════ FIND WORKER FLOW ═══════════════ */
// // // function FindWorkerFlow({ t, lang, step, jobRequest, upd, onSubmit, workerAccepts, incomingOffers, confirmedWorker, onConfirmWorker, onDismissWorker, onAcceptOffer, onRejectOffer, onCounterOffer, onReset, searchTimer, submitError, submitting, employerId }) {
...
// // //             <JobMap lat={jobRequest.lat} lng={jobRequest.lng} height={200} label={t.jobSiteLocation} showLiveWorkerLocation={true} liveWorkerId={confirmedWorker.workerId} />
// // //           </div>
// // //         )}
...
// //   console.log("🔌 Employer socket initialized on http://localhost:5000");
...
// //     postJob: "Post a Job Request", postJobSub: "Workers near you will see your request — like InDrive",
...
// //   // ── Job flow state ──
// //   const [step, setStep]                 = useState("form");  // form | searching | tracking
// //   const [jobRequest, setJobRequest]     = useState(BLANK);
// //   const [incomingOffers, setIncomingOffers] = useState([]);
// //   const [workerAccepts, setWorkerAccepts]   = useState([]);
// //   const [confirmedWorker, setConfirmedWorker] = useState(null); // { workerId, workerName, workerPhone, workerRating, price }
// //   const [activeRequestId, setActiveRequestId] = useState(null);
// //   const [searchTimer, setSearchTimer]   = useState(0);
// //   const [submitError, setSubmitError]   = useState("");
// //   const [submitting, setSubmitting]     = useState(false);
...
// //       const p = JSON.parse(u);
// //       setUser(p);
// //       console.log("👔 Employer joining socket with ID:", p.id);
// //       socket.emit("join", p.id);
// //       console.log("👔 Join event emitted for employer:", p.id);
...
// //   useEffect(() => {
// //     // Worker accepted the job (wants to do it)
// //     socket.on("worker_job_accept", (data) => {
// //       setWorkerAccepts(prev => {
// //         const exists = prev.find(w => w.workerId === data.workerId);
// //         return exists ? prev : [...prev, data];
...
// //     // Worker sent a counter price offer
// //     socket.on("worker_offer", data => {
// //       setIncomingOffers(prev => {
// //         const ex = prev.find(o => o.workerId === data.workerId);
// //         return ex ? prev.map(o => o.workerId === data.workerId ? data : o) : [...prev, data];
...
// //     // Worker confirmed by employer via "employer_accepted" flow (counter offer path)
// //     socket.on("worker_accepted", data => {
// //       setConfirmedWorker(data);
// //       setStep("tracking");
// //     });
...
// //       setFirstJobPosted(true);
// //       try {
// //         await fetch("http://localhost:5000/api/job-requests", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...jobRequest, employerId: user.id }) });
// //       } catch (_) {}
// //       socket.emit("new_job_request", { requestId: data._id, employerId: user.id, employerName: user.name, ...jobRequest, lat: jobRequest.lat, lng: jobRequest.lng });
// //       setStep("searching");
// //     } catch (_) {
// //       const fakeId = "demo_" + Date.now();
// //       setActiveRequestId(fakeId);
// //       setFirstJobPosted(true);
// //       socket.emit("new_job_request", { requestId: fakeId, employerId: user?.id, employerName: user?.name, ...jobRequest, lat: jobRequest.lat, lng: jobRequest.lng });
// //       setStep("searching");
// //     }
// //     setSubmitting(false);
// //   };

// //   /* ── Confirm a worker who directly accepted ── */
// //   const handleConfirmWorker = (worker) => {
// //     try {
// //       fetch(`http://localhost:5000/api/job-requests/${activeRequestId}/accept`, {
// //         method: "POST", headers: { "Content-Type": "application/json" },
...
// //       finalPrice: jobRequest.offeredPrice,
// //     });
// //     const confirmed = { ...worker, price: jobRequest.offeredPrice };
// //     setConfirmedWorker(confirmed);
// //     setStep("tracking"); // ← go straight to tracker
// //   };
...
// //   /* ── Accept a worker's counter offer ── */
// //   const handleAcceptOffer = async offer => {
// //     try {
// //       await fetch(`http://localhost:5000/api/job-requests/${activeRequestId}/accept`, {
// //         method: "POST", headers: { "Content-Type": "application/json" },
...
// //         .act-btn:hover{opacity:.88;transform:translateY(-1px)}
// //         ::-webkit-scrollbar{width:5px}::-webkit-scrollbar-thumb{background:#cbd5e1;border-radius:6px}
// //         @media(max-width:600px){.form-4col{grid-template-columns:repeat(2,1fr)!important}.form-2col{grid-template-columns:1fr!important}.main-pad{padding:14px!important}}
// //       `}</style>
...
// //             {/* ── JOB TRACKER (shown regardless of tab when tracking) ── */}
// //             {step === "tracking" && confirmedWorker && (
// //               <JobTracker
// //                 role="employer"
// //                 job={{ requestId: activeRequestId, title: jobRequest.title, category: jobRequest.category, workLocation: jobRequest.workLocation, lat: jobRequest.lat, lng: jobRequest.lng }}
...
// //               />
// //             )}
// //             {step !== "tracking" && activeTab === "history" && <EmptyState icon="🕐" title={t.noHistory} sub={t.noHistorySub} />}
// //           </div>
// //         </main>
// //       </div>
// //     </>
...
// //     { key: "emailVerified", label: t.stepEmailVerified, done: !!(userProfile.isEmailVerified),                                      points: 15, action: null },
// //     { key: "firstJob",      label: t.stepFirstJob,      done: firstJobPosted,                                                       points: 15, action: onGoPostJob },
// //     { key: "verified",      label: t.stepVerified,      done: !!(userProfile.isVerified),                                           points: 15, action: null },
// //   ];
...
// // /* ═══════════════ FIND WORKER FLOW ═══════════════ */
// // function FindWorkerFlow({ t, lang, step, jobRequest, upd, onSubmit, workerAccepts, incomingOffers, onConfirmWorker, onDismissWorker, onAcceptOffer, onRejectOffer, onCounterOffer, onReset, searchTimer, submitError, submitting }) {
// //   const [counterPrices, setCounterPrices] = useState({});
// //   const [generating, setGenerating]       = useState(false);
// //   const [geocoding, setGeocoding]         = useState(false);
...
// //     setGenerating(true);
// //     try {
// //       const res  = await fetch("http://localhost:5000/api/ai/generate-description", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ category: jobRequest.category, title: jobRequest.title, location: jobRequest.workLocation, budget: jobRequest.offeredPrice, lang }) });
// //       const data = await res.json();
// //       if (data.description) upd("description", data.description);
...
// //   const m = { pending:["#fef3c7","#d97706"], confirmed:["#dcfce7","#16a34a"], cancelled:["#fee2e2","#dc2626"], completed:["#dbeafe","#2563eb"], negotiating:["#faf5ff","#7c3aed"] };
// //   const [bg,color] = m[status]||m.pending;
// //   return <span style={{ background:bg, color, padding:"4px 11px", borderRadius:20, fontSize:11.5, fontWeight:700, whiteSpace:"nowrap" }}>{status}</span>;
// // }
// // function EmptyState({ icon, title, sub }) {
// //   return (
// //     <div style={{ maxWidth:400, margin:"32px auto", textAlign:"center" }}>
...
// /* ═══════════════ REQUESTS TAB (Employer) – Duplication Fixed Final Version ═══════════════ */
// function RequestsTab({ userId, t, userProfile, firstJobPosted, onGoPostJob }) {
...
//     // Active Jobs (only active status)
//     fetch(`http://localhost:5000/api/jobs/employer/${userId}`)
//       .then(r => r.json())
//       .then(data => {
//         const activeOnly = Array.isArray(data) 
//           ? data.filter(job => job.status === "active" || job.status === undefined || job.status === null)
//           : [];
//         setActiveJobs(activeOnly);
//       })
//       .catch(() => setActiveJobs([]));

//     // History Jobs (only completed)
//     fetch(`http://localhost:5000/api/jobs/employer/${userId}?status=completed`)
//       .then(r => r.json())
//       .then(data => {
//         const completedOnly = Array.isArray(data) ? data : [];
//         setHistoryJobs(completedOnly);
//       })
...
//       {/* Tabs */}
//       <div style={{ display: "flex", gap: 12, margin: "30px 0 20px", borderBottom: "1px solid #e2e8f0" }}>
//         <button onClick={() => setActiveTab("active")}
//           style={{ padding: "12px 28px", borderRadius: 12, background: activeTab === "active" ? "#3b82f6" : "#f1f5f9", color: activeTab === "active" ? "#fff" : "#475569", fontWeight: 600 }}>
//           My Active Requests
//         </button>
//         <button onClick={() => setActiveTab("history")}
//           style={{ padding: "12px 28px", borderRadius: 12, background: activeTab === "history" ? "#3b82f6" : "#f1f5f9", color: activeTab === "history" ? "#fff" : "#475569", fontWeight: 600 }}>
//           Job History
//         </button>
//       </div>
...
//         <div style={{ background:"#fff", borderRadius:16, padding:80, textAlign:"center" }}>
...
//           {activeJobs.map(job => (
//             <div key={job._id} style={{ background: "#fff", borderRadius: 16, padding: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
//               <h4 style={{ fontWeight: 700 }}>{job.title}</h4>
//               <p style={{ color: "#64748b" }}>{job.workLocation}</p>
//             </div>
//           ))}
//         </div>
//       )}
...
//               <div key={job._id || index} style={{ background: "#fff", borderRadius: 16, padding: 20, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
//                 <h4 style={{ fontWeight: 700 }}>{job.title}</h4>
//                 <p style={{ color: "#64748b" }}>{job.workLocation}</p>
//                 {job.finalPrice && <p style={{ color: "#16a34a", marginTop: 8 }}>Final Price: Rs. {job.finalPrice}</p>}
//               </div>
//             ))}
//           </div>
//         )
...
export default function EmployerDashboard() {
  ...
  const handleSendRequest = async e => {
    ...
    try {
      ...
      try {
        const jrRes  = await fetch("http://localhost:5000/api/job-requests", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...jobRequest, employerId: user.id }) });
        const jrData = await jrRes.json();
        if (jrData._id) requestId = jrData._id;
      } catch (_) {}
      setActiveRequestId(requestId);
      socket.emit("new_job_request", { requestId, employerId: user.id, employerName: user.name, ...jobRequest, lat: jobRequest.lat, lng: jobRequest.lng });
      setStep("searching");
    } catch (_) {
      const fakeId = "demo_" + Date.now();
      setActiveRequestId(fakeId);
        ...
        <main style={{ flex: 1, overflow: "auto", minWidth: 0 }}>
          ...
          <div className="main-pad" style={{ padding: "24px 26px" }}>
            {step === "tracking" && confirmedWorker && (
              <JobTracker
                role="employer"
                job={{ requestId: activeRequestId, title: jobRequest.title, category: jobRequest.category, workLocation: jobRequest.workLocation, lat: jobRequest.lat, lng: jobRequest.lng, jobDuration: jobRequest.jobDuration }}
                worker={confirmedWorker}
                employer={{ employerId: user.id, employerName: user.name, employerPhone: user.phone }}
                agreedPrice={confirmedWorker.price || jobRequest.offeredPrice}
                socket={socket}
            ...
            {step !== "tracking" && activeTab === "requests" && (
              <RequestsTab
                ...
                firstJobPosted={firstJobPosted} onGoPostJob={() => setActiveTab("find")}
              />
            )}
...
function RequestsTab({ userId, t, userProfile, firstJobPosted, onGoPostJob }) {
  ...
  const fetchRequests = () => {
    if (!userId) return;
    setLoading(true);
    // Fetch only from job-requests (the real-time flow records)
    fetch(`http://localhost:5000/api/job-requests/employer/${userId}`)
      .then(r => r.json())
      .then(data => {
        // Filter active (non-completed) requests
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        ...
        <button onClick={fetchRequests} style={{ padding: "8px 16px", borderRadius: 10, border: "1.5px solid #e2e8f0", background: "#fff", cursor: "pointer", fontSize: 12, color: "#64748b", fontWeight: 600 }}>
          🔄 Refresh
        </button>
      </div>
      ...
      {requests.length === 0 ? (
        <div style={{ background: "#fff", borderRadius: 16, padding: "60px 24px", textAlign: "center" }}>
          ...
          <button onClick={onGoPostJob}
            style={{ padding: "12px 24px", borderRadius: 12, border: "none", background: "linear-gradient(135deg,#3b82f6,#2563eb)", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>
            🔍 Find a Worker Now
          </button>
        </div>
      ) : (
            ...
            return (
              <div key={req._id} style={{ background: "#fff", borderRadius: 18, padding: 22, boxShadow: "0 2px 14px rgba(0,0,0,0.07)", border: `1.5px solid ${sc.border}` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                  <div style={{ flex: 1 }}>
                    <h4 style={{ fontSize: 17, fontWeight: 800, color: "#0f172a", margin: "0 0 4px" }}>
                      {req.title || req.category || "Job Request"}
                    </h4>
                    <span style={{ fontSize: 12, color: "#64748b", fontWeight: 600, background: "#f1f5f9", padding: "2px 10px", borderRadius: 20 }}>
                      {req.category}
                    </span>
                  </div>
                  <span style={{ padding: "5px 12px", borderRadius: 20, fontSize: 11, fontWeight: 800, background: sc.bg, color: sc.color, flexShrink: 0, marginLeft: 12 }}>
                    {sc.label}
                  </span>
                </div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: req.acceptedWorker ? 14 : 0 }}>
                  ...
                  {req.urgency && (
                    <span style={{ fontSize: 12, color: "#f59e0b", fontWeight: 700, background: "#fef3c7", padding: "3px 10px", borderRadius: 20 }}>
                      {{ "1_hour": "⚡ Within 1 Hour", "today": "📅 Today", "this_week": "📆 This Week", "flexible": "🕐 Flexible" }[req.urgency] || req.urgency}
                    </span>
                  )}
                  {req.jobDuration && (
                    <span style={{ fontSize: 12, color: "#6366f1", fontWeight: 600, background: "#f5f3ff", padding: "3px 10px", borderRadius: 20, display: "flex", alignItems: "center", gap: 4 }}>
                      ⏱ {req.jobDuration}
                    </span>
                  )}
                  {req.offeredPrice && (
                    <span style={{ fontSize: 12, color: "#16a34a", fontWeight: 700, background: "#f0fdf4", padding: "3px 10px", borderRadius: 20 }}>
                      💰 Rs. {req.offeredPrice}
                    </span>
                  )}
                  {req.finalPrice && req.finalPrice !== req.offeredPrice && (
                    <span style={{ fontSize: 12, color: "#1d4ed8", fontWeight: 700, background: "#eff6ff", padding: "3px 10px", borderRadius: 20 }}>
                      🤝 Agreed: Rs. {req.finalPrice}
                    </span>
                  )}
                </div>
                ...
                {req.acceptedWorker && (
                  <div style={{ background: "linear-gradient(135deg,#f0fdf4,#dcfce7)", borderRadius: 12, padding: "12px 16px", border: "1.5px solid #86efac", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 38, height: 38, borderRadius: "50%", background: "linear-gradient(135deg,#22c55e,#16a34a)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, fontWeight: 800, color: "#fff", flexShrink: 0 }}>
                        {(req.acceptedWorker.name || "W").charAt(0).toUpperCase()}
                      </div>
                      <div>
                    ...
                    {req.acceptedWorker.phone && (
                      <a href={`tel:${req.acceptedWorker.phone}`}
                        style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", borderRadius: 10, background: "#22c55e", color: "#fff", textDecoration: "none", fontSize: 13, fontWeight: 700 }}>
                        📞 {req.acceptedWorker.phone}
                      </a>
                    )}
                  </div>
      ...
      <div style={{ background: "linear-gradient(135deg,#0b1526,#1a3255)", borderRadius: 18, padding: "18px 24px", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 800 }}>
            Rs. {history.reduce((sum, r) => sum + Number(r.finalPrice || r.offeredPrice || 0), 0).toLocaleString()}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {history.map(req => (
          <div key={req._id} style={{ background: "#fff", borderRadius: 18, padding: 22, boxShadow: "0 2px 14px rgba(0,0,0,0.07)", border: "1.5px solid #dcfce7" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: req.acceptedWorker ? 14 : 0 }}>
              {req.jobDuration && (
                <span style={{ fontSize: 12, color: "#6366f1", fontWeight: 600, background: "#f5f3ff", padding: "3px 10px", borderRadius: 20 }}>⏱ {req.jobDuration}</span>
              )}
              {(req.finalPrice || req.offeredPrice) && (
                <span style={{ fontSize: 13, fontWeight: 800, color: "#16a34a" }}>💰 Rs. {req.finalPrice || req.offeredPrice}</span>
              )}
            </div>
            ...
            {req.acceptedWorker && (
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", background: "#f0fdf4", borderRadius: 10, border: "1px solid #bbf7d0" }}>
                <div style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg,#22c55e,#16a34a)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 800, color: "#fff", flexShrink: 0 }}>
                  {(req.acceptedWorker.name || "W").charAt(0).toUpperCase()}
                </div>
                <div>
        ...
        <form onSubmit={onSubmit} style={{ padding: "26px 26px 30px" }}>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input
                className="inp"
                value={jobRequest.jobDuration}
                onChange={e => upd("jobDuration", e.target.value)}
                placeholder={t.jobDurationPlaceholder}
